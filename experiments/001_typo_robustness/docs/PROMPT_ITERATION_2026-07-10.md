...
to ≥0.95. Method: three smoke-test rounds on a Colab T4, each running all 200
clean reasoning items (100 GSM8K + 100 GSM-Symbolic) per prompt variant
through the repo's own `VllmEngine` (greedy, `max_new_tokens=512`) and the
frozen scorer (`scoring.extract_reasoning_answer`). Compliance = share of
generations whose answer extracted at the `hash_delimited` tier — the same
...
Finding: the post-question reminder alone is worth ~+11pp; **more inline
exemplars hurt** (v2 < v0). Inline few-shot plateaus near 0.77.
...
with exemplar count; the reminder is still worth ~+2.5pp at 3-shot. `r3` gives
~4pp headroom over the 0.95 gate before pooling perturbed rows. Accuracy 0.250
vs the 0.285 1-shot baseline is within sampling noise at n=200 (SE ≈ 0.03).
