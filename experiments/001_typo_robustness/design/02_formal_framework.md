...
This document defines the mathematics the rest of the suite refers to. Every metric used anywhere in the paper is defined here once, so scoring code and statistics code have a single source of truth. Notation follows Document 00 §0.7.
...
With this basis, the edit budget `k` is an exact, auditable count of primitive operations, not an approximate percentage. This is the foundation of every "the numbers are not arbitrary" argument in Document 06.
...
This is the minimal complete representation: removing any component merges conditions the experiment must keep distinct (one deletion in "please" is not one deletion in "France"; a keyboard-neighbor substitution is not a uniform-random one; `k=1` is not `k=8`). The state vector is logged on every output row (Document 08 §8.4) together with an **edit script** (the exact list of operations with character indices and before/after characters), so `x'` is fully reconstructible from `x` and `r`. Reconstructibility is what makes the perturbation set auditable rather than a black box.
...
```
Example: "capital of *France*" → "capital of *Germany*" → Berlin. *This is the over-invariance control, not a robustness test.* It measures whether a model is *appropriately* sensitive rather than blindly invariant.
...
R(r) = A₁(r) / A₀
```
Fraction of clean competence surviving. Normalizes degradation by baseline, so different models (and quantized vs fp16) are comparable even when their clean accuracies differ.
...
Three deliberate limits keep the formal claims defensible:
