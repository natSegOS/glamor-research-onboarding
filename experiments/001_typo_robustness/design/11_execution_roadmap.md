...
A stage-gated plan from where the repo is now to a submittable paper. Each stage has a concrete deliverable and a **quantitative decision threshold** that says whether to proceed, revise, or branch. The gates are what keep the project from burning compute on a design that a pilot would have shown to be underpowered or a novelty search would have shown to be closed.
...
**Do:** build the `src/` modules (Doc 08), wire vLLM (Doc 07), write the unit tests (Doc 08 §8.7). Then run the pilot: Llama-3.2-1B-Instruct on 200 paired GSM-Symbolic items at `k=1`, Regime A, keyboard-neighbor.
...
**Do:** generate the full perturbation set; audit 385 stratified items per regime with 3 annotators (Doc 09).
...
1. Module 1 (mediation) — the primary contribution; run first and in full.
2. Module 3 regimes A/C (selectivity + over-robustness).
3. Module 2 (quantization) including the fp16 arm on the A40 tier.
4. Module 3 regime B; Module 4 (descriptive); exploratory `k=8`, sampling check, byte-level reference.
...
**Venue selection:**
- **Primary target: ACL Rolling Review → EMNLP 2026 main** (the "evaluation and analysis" / "interpretability" tracks reward exactly this mechanism-plus-rigor profile).
- **Alternatives:** NAACL 2027 main; NeurIPS 2026 Datasets & Benchmarks (if the released perturbation set + audited regimes are framed as a reusable resource); ACL Findings as a strong fallback.
- **A single-pillar short version** (mediation only, fallback-T4 study) fits a workshop or short-paper track and is the safety net if compute or time collapses.
...
| 1–2 | 1 | `src/` + tests + vLLM; pilot; **N fixed, throughput measured, A₀ validated** |
