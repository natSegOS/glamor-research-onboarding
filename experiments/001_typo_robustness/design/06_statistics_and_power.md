...
- **Convergence contingencies (fixed in advance):** if the maximal model is singular or fails to converge — common with only five models — we (1) drop random-slope correlations (`||` syntax), then (2) drop the by-model random slope, then (3) if `model` random effects remain unstable, treat `model` as a *fixed* categorical effect (five levels is borderline for a random effect anyway; Gelman & Hill's rule of thumb is ≥5–8 levels). The fallback ladder is pre-registered so the choice is not seen as cherry-picking.
  - **Implementation status:** `fit_crossed_mixed_effects_logistic` (`src/analysis/models.py`) does not yet implement steps (1)–(2) of this structural ladder — `statsmodels.MixedLM`'s formula interface has no direct equivalent of lme4's `||` uncorrelated-random-slopes syntax. What it implements today is a different, simpler fallback: retry the *same* maximal model with two different numerical optimizers (statsmodels' own quasi-Newton cascade, then Nelder-Mead), then fall to step (3) above (GLM with item/model as fixed factors) if neither converges. Step (3) is implemented correctly and matches this section. Implementing the true structural ladder (steps 1–2) is deferred to before the confirmatory main-study run — see `ConvergenceMethod` in `src/enums.py` for the current, honestly-labeled implementation.
